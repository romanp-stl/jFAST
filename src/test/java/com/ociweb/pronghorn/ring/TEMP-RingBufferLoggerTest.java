...
public class RingBufferLoggerTest implements RingBufferLoggerMessageConsumer {
	...
	private static final int BYTE_RING_SIZE = 16;
    ...
    public void testLoggingDirrectlyToSystemError() {
    	...
    	initiateLogger("Output Directly to System.Error", 
    					"test1", BYTE_RING_SIZE, null, null);
    ...
    public void testLoggingToALocalRingAndReadingHere() {
    	...
    	final byte byteRingSizeInBits = BYTE_RING_SIZE;
    	RingBuffer loggerRing = new RingBuffer(primaryRingSizeInBits,byteRingSizeInBits);
	   ...
	   Thread t = new Thread(new Runnable(){
			...
			public void run() {
		        while (true) {
		        	...
		        	if(RingWalker.tryReadFragment(ring)) {
			        	RingReader.readASCII(ring, 0, target);
    ...
    * 1) Directly To File 		(toRing=false, ring=null, consumer=null). This mode is equivalent to SimpleLogger.
    * 2) To Ring, Then To File	(toRing=true, ring=null, consumer=null). Logger writes messages to ring and then 
    *                                                                    consume them and writes to a file.
    * 3) To Ring, Then to Consumer (toRing=true, ring=null. consumer=this). Logger writes messages to ring and then 
    *                                                                    read them and call consumer.
    * 4) To external ring (toRing=false, ring!=null, consumer=null). Logger just write messages to the provided ring.
   ...
   private void initiateLogger(String title, String name, 
	...
	@Override
	public void consumeMessage(StringBuilder message) {
		message.append(" |");
